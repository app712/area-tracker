...
function optimizeAndProcessCsv(sheetId, csvData, defaultStatus) {
  ...
  for (var i = 0; i < csvData.length; i++) {
    ...
    if (!optimizedName) {
      // 先頭を^で固定しないと、市区町村より前（都道府県など）が住所側から消えてしまう。
      // 番地末尾の裸の数字（例：「1-2-3」の"3"）は、号室・階などのキーワードが続く場合のみ建物名扱いにし、
      // そうでなければ番地の一部とみなして分割しない（誤って番地を建物名として切り離すのを防ぐ）。
      var splitPattern = /^(.+?\d+[-－]?\d*[-－]?\d*)([ \u3000].+|[A-Za-z].*|\d+(?:号室|号棟|号|棟|階|F|f)\S*|[階号棟室].*|[①-⑨].*|[^0-9\-－市区町村郡].+(?:ビル|マンション|アパート|ハイツ|コーポ|メゾン|レジデンス|ライオンズ|シャトー|プラザ|コート|ビルディング|エステート|ヴィラ|パレス|ステージ|テラス|スクエア|タワー|ルーム|H|h|F|f|号|室))$/;
